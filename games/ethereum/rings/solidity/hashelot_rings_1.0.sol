...
contract hashelot_rings{ // Son of a lockdown
    ...
    function closeBet() public payable {
        ...
        currentKey = uint(keccak256(abi.encodePacked(blockhash(stackTime+stackWait), stackPlayers[0])));
        _winnerKey = stackPlayers[0];

        for (uint k = 1; k < stackPlayers.length; k++){
          if(uint(keccak256(abi.encodePacked(blockhash(stackTime+stackWait), stackPlayers[k]))) < currentKey){
            currentKey = uint(keccak256(abi.encodePacked(blockhash(stackTime+stackWait), stackPlayers[k])));
